...
public class Erp105005Controller {
     ...
     * @return 처리 결과
     */
    @RequestMapping(value = {"erp105005UpdateEtcRemark"}, method = RequestMethod.POST)
    @ResponseBody
    public Map<String, Object> erp105005UpdateEtcRemark(Erp105005VO erp105005VO) {
        log.info("기타비고 업데이트 요청");
        log.info("처리 정보: {}", erp105005VO);
        ...
        Map<String, Object> result = new HashMap<>();
        try {
            int processedCount = erp105005Service.erp105005UpdateEtcRemark(erp105005VO);
            result.put("rtnYn", "Y");
            result.put("rtnMsg", "기타비고가 저장되었습니다.");
            result.put("processedCount", processedCount);
            log.info("기타비고 업데이트 성공: {}건 처리", processedCount);
        } catch (Exception e) {
            result.put("rtnYn", "N");
            result.put("rtnMsg", "처리 중 오류가 발생했습니다.");
