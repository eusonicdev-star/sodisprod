package com.allianceLogistics.alsys.erp.erp101016.entity;

import java.util.List;

import com.allianceLogistics.alsys.common.entity.commonVO;

import lombok.Getter;
import lombok.Setter;
import lombok.ToString;


@Getter
@Setter
//@ToString
@ToString(callSuper=true)
public class Erp101016VO extends commonVO{
		
	private static final long serialVersionUID = 1L;

	private String  agntCd;     //화주코드
	private String  agntNm;     //화주명칭
	private String  soType;     //오더유형코드
	private String  soTypeNm;       //오더유형명칭
	private String  soStatCd;       //오더상태코드
	private String  soStatNm;       //오더상태명칭
	private String  dcCd;       //물류센터코드
	private String  dcNm;       //물류센터코드
	private String  instCtgr;       //시공카테고리코드
	private String  instCtgrNm;     //시공카테고리명칭
	private String  ordrInptDt;     //시스템등록일
	private String  rcptDt;     //고객접수일
	private String  rqstDt;     //고객배숑요청일
	private String  dlvyCnfmDt;     //배송확정일
	private String  giLiftDt;       //공장출고일
	private String  mallCd;     //쇼핑몰
	private String  brand;      //브랜드
	private String  soNo;       //AL오더번호
	private String  refSoNo;        //고객사주문번호
	private String  acptEr;     //수취인
	private String  acptTel1;       //수취인휴대번호
	private String  acptTel2;       //수취인일반번호
	private String  postCd;     //우편번호
	private String  addr1;      //주소
	private String  addr2;      //상세주소
	private String  dlvyRqstMsg;        //배송메세지
	private String  custSpclTxt;        //고객사특이사항
	private String  dlvyCostType;       //운임구분코드
	private String  dlvyCostTypeNm;     //운임구분명칭
	private String  rcptCost;       //착불비
	private String  passCost;       //통행료
	private String  instDt;     //시공일
	private String  paltNoxx;       //팔렛트번호
	private String  instEr;     //시공기사
	private String  team;       //시공기사(팀)
	private String  instMemo;       //기사메모
	private String  instCost;       //시공비
	private String  dlvyCost;       //물류비
	private String  tblSoMId;       //AL오더고유ID
	private String  missCloseYn;        //미마감조치유무
	private String  missCloseType;      //미마감조치유형코드
	private String  missCloseTypeNm;        //미마감조치유형명칭
	
	
	        
	private String  dtType;		       //날짜유형검색  
	private String  fromDt;		       //달력 검색 시작일
	private String  toDt;		       //달력 검색 종료일 
	private String  dcList;		        //물류센터 
	private String  acptNm;		        //수령인검색 
	private String  addr;			        //주소검색 
	private String  acptTel;		        //전화번호검색 
	private String  soList;		        	//AL오더번호 검색 
	private String  refSoList;	        	//고객사 주문번호 검색
	private String  instCtgrList;	    	//시공카테고리 
	private String  soTypeList;	        	//오더유형리스트 
	private String  soStatCdList;	    	//오더상태리스트 
	private String  useYn;		        	//조치유무 
	private String  missCloseTypeList;		//미마감유형 
	
	private String  orgnSoNo;
	private String  closeStatNm;
	
	private	String[] midValueArray;
	
	private	String	rtnSoType;	//원오더의 오더유형코드 말고 요청하는 오더유형코드
	private	String	rtnSoStatCd;	//팝업화면에 넣을 주문상태
	
	private	String	fromEr			;		//발송인
	private	String	fromTel1		;
	private	String	fromTel2		;
	private	String	fromPostCd		;
	private	String	fromAddr1		;
	private	String	fromAddr2		;
	private	String	distChanCd		;

	private	String	delResn			;
	private	String	memo			;		//비고

	private	String	agntList		;		//화주

	private	String	soStatList		;		//오더상태
	private	String	mallNm			;		//쇼핑몰
	
	private	String	prodSeq			;		//순번
	private	String	prodCd			;		//상품코드
	private	String	prodNm			;		//상품명
	private	String	qty				;		//수량

	private	String	ordrEr			;		//주문자
	private	String	dlvyLiftDt		;		//공장출고일
	
	private	String	srchDay			;		//찾을날짜 범위
	private	String	prodCdList		;		//상품코드 리스트
	private	String	qtyList			;		//수량리스트
	
	//배송확정일 가능한 날짜 관련 
	private	String	capaId		;		//좌석아이디
	private	String	weekCd		;		//요일코드
	private	String	weekNm		;		//요일명칭
	private	String	capaCtgr	;		//시공카테고리코드
	private	String	capaCtgrNm	;		//시공카테고리명칭
	private	String	capaType	;		//시공유형코드
	private	String	capaTypeNm	;		//시공유형명칭
	private	String	zoneType	;		//권역유형코드
	private	String	zoneTypeNm	;		//권역유형명칭
	private	String	zoneCd		;		//권역코드
	private	String	zoneCdNm	;		//권역명칭
	private	String	ableYn		;		//날짜 사용가능
	private	String	totCapa		;		//총시공케파
	private	String	useCapa		;		//사용한케파
	private	String	remnCapa	;		//남은케파

	private	List<Erp101016VO>	detailData;

	private	String	costType	;		//운임구분
	private	String	inptSys		;		//입력시스템 (1건식 입력 ERP_SO, 엑셀로 한꺼번에 입력 ERP_EXCEL)
	private	String	tblSoPId	;		//상세 고유 아이디
	private String	rtnName;

	private	String	srchAddr;			//주소검색

	private	String	missCloseStep;		//미마감 처리단계
	private	String	endCloseStep;		//미마감 최종단계
	private	String	orgnSoMId;			//미마감 주문 입력시 넣는 원 AL 오더고유ID
	private	String	custNm;				//미마감 헤더 입력 후 리턴받는 고객명
	private	String	rtnBtnType;			//어떤 버튼을 눌렀는가
	
}