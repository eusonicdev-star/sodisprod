...
$.ajaxSetup({
    ...
    error: function (request, status, error) {
        ...
        if (request.status == '403') {
            ...
            alert("로직 수행 실패 : " + request.status + ", " + error + "\n" + decodeURL(request.responseText));
            return;
        }
    }
});
...
function get_chk_json(myGrid) {
    ...
    for (var r = 0; r < rows; r++) {
        ...
        for (var c = 0; c < columns; c++) {
            if (myGrid.cellByIndex(r, chkCoulumId).getValue() == 1)//체크되어있으면
            {
...
function get_cud_json(myGrid) {
    ...
    for (var r = 0; r < rows; r++) {
        ...
        for (var c = 0; c < columns; c++) {
            if (myGrid.cellByIndex(r, cudCoulumId).getValue())	//cud 값이 있으면
            {
                rowArr[myGrid.getColumnId(c)] = myGrid.cellByIndex(r, c).getValue();
            }
        }
...
//팝업창 열기
function myWindowsOpen(windowId, windowText, windowUrl, windowLeft, windowTop, windowWidth, windowHeight, windowsModal, windowOnClose, windowSendObject) {
    myWins.createWindow({
        id: windowId,
        left: windowLeft,
        top: windowTop,
...
//팝업창 열기 window 객체 포함
function myWindowsOpen2(myWins, windowId, windowText, windowUrl, windowLeft, windowTop, windowWidth, windowHeight, windowsModal, windowOnClose, windowSendObject) {
    myWins.createWindow({
        id: windowId,
        left: windowLeft,
        top: windowTop,
...
function get_data(myGrid) {
    ...
    for (var r = 0; r < rows; r++) {
        ...
        for (var c = 0; c < columns; c++) { //iterate through the columns
...
function pageUrl() {
    ...
    var pageName = "";
    if (thisfilefullname.lastIndexOf("?") < 1)	//URL 뒤에 ?로 전달되는 GET방식이 아니라면
    {
...
function get_json_res(myGrid, res) {
    ...
    for (var r = 0; r < rows; r++)		//조회한 결과 res 의 로우 수 만큼 반복
    {
        for (var c = 0; c < columns; c++) {
            var isHidden = myGrid.isColumnHidden(c)
            if (!isHidden)	//false 면 컬럼이 가려지지 않았다는것
            {
...
function dateTypeChange(ymd, len) {
    ...
    if (len == 8)	//날짜가 뭐가 들어오던 8자리 날짜형태 yyyyMMdd 로 변경
    {
        ymd = ymd.replace(/-/g, "");
