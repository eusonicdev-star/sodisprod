package com.sonictms.alsys.erp.erp101001.service;

import com.sonictms.alsys.common.entity.ErpCommonVO;
import com.sonictms.alsys.common.mapper.ErpCommonMapper;
import com.sonictms.alsys.common.service.*;
import com.sonictms.alsys.common.util.AddressNormalizer;
import com.sonictms.alsys.erp.erp101001.entity.Erp101001VO;
import com.sonictms.alsys.erp.erp101001.mapper.Erp101001Mapper;
...
public class Erp101001Service {
    ...
    public Erp101001VO erp101001p1Save(Erp101001VO erp101001VO) {
        ...
        if (returnVo.getTblSoMId() != null && returnVo.getTblSoMId() != "" && returnVo.getSoNo() != null && returnVo.getSoNo() != "") {
            ...
            if (errCnt > 0) { // 에러가 있으면
                ...
                // 강제롤백
                System.out.println("강제롤백발생 : TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()");
    ...
    public Erp101001VO erp101001p5Save(Erp101001VO erp101001VO) {
        ...
        // 2. 헤더에서 리턴 받은 고유 아이디 와 AL 오더
        System.out.println("반품주문 저장-헤더- 사용자      - headerVo.getSaveUser() : " + erp101001VO.getSaveUser());
        System.out.println("반품주문 저장-헤더- 주문 고유 ID - headerVo.getTblSoMId() : " + returnVo.getTblSoMId());
        System.out.println("반품주문 저장-헤더- 주문 번호    - headerVo.getSoNo()     : " + returnVo.getSoNo());
    ...
    public Erp101001VO erp101001p3Updt(Erp101001VO erp101001VO) {
        ...
        if (erp101001VO.getOutCmpyCd().equals("KPP")) {
            System.out.println("주문상태 수정 - kpp 오더입니다.");
            ...
            System.out.println(erpCommonVO.toString());
        ...
        // 2. 헤더에서 리턴 받은 고유 아이디
        if (returnVo != null) {
            System.out.println("주문 수정-헤더- 사용자      - headerVo.getSaveUser() : " + erp101001VO.getSaveUser());
            System.out.println("주문 수정-헤더- 주문 고유 ID - headerVo.getTblSoMId() : " + returnVo.getTblSoMId());
            System.out.println("주문 수정-헤더- 주문 번호    - headerVo.getSoNo()     : " + returnVo.getSoNo());
        }
        // 헤더수정이 성공했으면 리턴값이 있다
        if (returnVo != null && returnVo.getTblSoMId() != null && returnVo.getTblSoMId() != "" && returnVo.getSoNo() != null
                && returnVo.getSoNo() != "") {
    ...
    public Erp101001VO erp101001ChkDel(Erp101001VO erp101001VO) {
        //20220517 정연호 kpp오더 라면 조회해서 api로 보낼 값 추출하기
        if (erp101001VO.getOutCmpyCd().equals("KPP")) {
            System.out.println("체크한 것 일괄삭제 - 주문상태 변경 - kpp 오더입니다.");
            ErpCommonVO erpCommonVO = new ErpCommonVO();
            erpCommonVO.setSoNoList(erp101001VO.getSoNo());
    ...
    @Autowired
    private AddressCoordService addressCoordService; // 좌표 관리용 서비스 (TBL_ADDR_COORD 담당)
    ...
    public int syncCoordinates(Erp101001VO searchVO) {
        int processCount = 0;
        List<Erp101001VO> noCoordList = erp101001Mapper.selectNoCoordList(searchVO);

        for (Erp101001VO order : noCoordList) {
            // 1. 기본 전처리
            String rawAddr = (order.getAddr1() + " " + (order.getAddr2() == null ? "" : order.getAddr2()))
                    .replaceAll("(?i)null", "").trim();
            if (rawAddr.isEmpty()) continue;

            try {
                Map<String, Object> coordMap = null;
                String finalSuccessKey = "";
                String finalApiType = "KAKAO";

                // 루프 시작 전 아파트 정보 초기화
                order.setAptNm("");
                order.setDongNm("");

                // 나중에 구글 검색 등을 위해 키들을 저장해둘 변수
                String aptKey = "";
                String formatKey = "";
                String jusoKey = "";

                // 아파트 정보 임시 저장용
                Map<String, String> aptInfo = AddressNormalizer.analyzeApartment(rawAddr);

                // =========================================================
                // [Step 1] 아파트/동 필살기 (최우선 순위!)
                // =========================================================
                if (aptInfo != null) {
                    aptKey = aptInfo.get("searchKey"); // 예: "강동구 선사현대아파트 101동"

                    log.info(">>> [Step 1] 아파트 우선 검색 시도: {}", aptKey);

                    // 1-1. 캐시 확인
                    coordMap = addressCoordService.selectCoordByKey(aptKey);

                    // 1-2. API 호출
                    if (coordMap == null) {
                        coordMap = kakaoMapService.getKakaoCoordinate(aptKey);
                    }

                    if (coordMap != null) {
                        finalSuccessKey = aptKey;
                        finalApiType = "KAKAO";

                        // ★ 성공 시 아파트 정보 저장
                        order.setAptNm(aptInfo.get("aptNm"));
                        order.setDongNm(aptInfo.get("dongNm"));
                    }
                }

                // =========================================================
                // [Step 2] 실패 시 -> 일반 포맷팅 검색
                // =========================================================
                if (coordMap == null) {
                    formatKey = AddressNormalizer.formatAddress(rawAddr); // 예: "... 101동(선사현대)"

                    // 아파트 키와 포맷팅 키가 같으면(정보가 없어서) 중복 검색 방지
                    if (!formatKey.equals(aptKey)) {
                        log.info(">>> [Step 2] 일반 포맷팅 검색 시도: {}", formatKey);

                        coordMap = addressCoordService.selectCoordByKey(formatKey);
                        if (coordMap == null) coordMap = kakaoMapService.getKakaoCoordinate(formatKey);

                        if (coordMap != null) {
                            finalSuccessKey = formatKey;
                            finalApiType = "KAKAO";
                        }
                    }
                }
                ...
                // =========================================================
                // [Step 3] 실패 시 -> Juso 표준화
                // =========================================================
                if (coordMap == null) {
                    jusoKey = jusoApiService.searchRoadAddress(rawAddr);

                    if (jusoKey != null && !jusoKey.isEmpty()) {
                        log.info(">>> [Step 3] Juso 표준화 검색 시도: {}", jusoKey);

                        coordMap = addressCoordService.selectCoordByKey(jusoKey);
                        if (coordMap == null) coordMap = kakaoMapService.getKakaoCoordinate(jusoKey);

                        if (coordMap != null) {
                            finalSuccessKey = jusoKey;
                            finalApiType = "KAKAO";
                        }
                    }
                }
                ...
                // =========================================================
                // [Step 4] 최후 -> Google Map
                // =========================================================
                if (coordMap == null) {
                    // 우선순위: Juso > Apt(필살기) > Format(일반)
                    String googleKey = "";
                    if (jusoKey != null && !jusoKey.isEmpty()) googleKey = jusoKey;
                    else if (!aptKey.isEmpty()) googleKey = aptKey;
                    else googleKey = formatKey;

                    log.info(">>> [Step 4] 구글 검색 시도 (Key: {})", googleKey);

                    coordMap = googleMapService.getCoordinate(googleKey);

                    if (coordMap != null) {
                        finalSuccessKey = googleKey;
                        finalApiType = "GOOGLE";

                        // (옵션) 구글이 '아파트 키'로 성공했다면, 아파트 정보를 넣어주는 게 이득
                        if (googleKey.equals(aptKey) && aptInfo != null) {
                            order.setAptNm(aptInfo.get("aptNm"));
                            order.setDongNm(aptInfo.get("dongNm"));
                        }
                    }
                ...
                // [Final] 저장
                // =========================================================
                if (coordMap != null && !finalSuccessKey.isEmpty()) {
                    saveCoordToCache(coordMap, finalSuccessKey, rawAddr, finalApiType);
                    updateOrderWithCoord(order, coordMap, finalSuccessKey);
                    processCount++;
                    log.info("좌표 동기화 성공! [{} -> {}]", rawAddr, finalSuccessKey);
                } else {
                    log.warn(">>> 최종 실패: {}", rawAddr);
                }

            } catch (Exception e) {
                log.error("Sync Error (SoNo: " + order.getSoNo() + ")", e);
            }
        }
        return processCount;
    }

    // Helper 메서드들은 기존과 동일하게 유지
    private void updateOrderWithCoord(Erp101001VO order, Map<String, Object> map, String successAddr) {
        Object x = map.containsKey("x") ? map.get("x") : map.get("lng");
        Object y = map.containsKey("y") ? map.get("y") : map.get("lat");

        if (x != null && y != null) {
            order.setMapX(String.valueOf(x));
            order.setMapY(String.valueOf(y));
            order.setSuccAddr(successAddr);
            // aptNm, dongNm은 Step 1에서 set 되었음
            erp101001Mapper.updateOrderCoord(order);
        }
    }

    // ---------------------------------------------------------
// [Helper] 캐시 저장 (TBL_ADDR_COORD)
// ---------------------------------------------------------
    private void saveCoordToCache(Map<String, Object> map, String key, String raw, String apiType) {
        try {
            // 이미 있는 키면 중복 에러가 날 수 있으므로 try-catch
            // 혹은 Query에서 MERGE INTO / ON DUPLICATE KEY UPDATE 처리 권장
            map.put("searchKey", key);
            map.put("rawAddr", raw);
            map.put("apiType", apiType);

            addressCoordService.insertCoord(map);
        } catch (Exception e) {
            // 이미 캐시에 존재하면 Pass
        }
    }
