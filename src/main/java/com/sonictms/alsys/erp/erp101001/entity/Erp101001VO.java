...
public class Erp101001VO extends CommonVO {
    ...
    // 1. DB 프로시저에서 반환하는 컬럼 (MAP_X, MAP_Y)
    private String mapX;
    private String mapY;

    // [추가] 성공한 주소 키
    private String succAddr;

    private String aptNm;
    private String dongNm;

    // 2. HTML 그리드에서 'coord'라는 이름으로 데이터를 찾을 때 호출되는 메서드
    // Lombok이 있어도 수동으로 getCoord를 만들면 이것이 우선 적용됩니다.
    public String getCoord() {
        if (this.mapX != null && !this.mapX.equals("") && this.mapY != null && !this.mapY.equals("")) {
            // 예: "127.12345, 37.12345" 형태로 묶어서 반환
