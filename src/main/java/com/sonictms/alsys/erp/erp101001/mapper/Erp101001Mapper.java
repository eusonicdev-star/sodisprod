...
public interface Erp101001Mapper {
    ...
    /**
     * [추가 1] 좌표가 없는(NULL) 주문 목록 조회
     * - XML ID: selectNoCoordList
     * - 용도: '좌표 확인' 버튼 클릭 시 업데이트 대상 추출
     */
    List<Erp101001VO> selectNoCoordList(Erp101001VO vo);

    /**
     * [추가 2] 찾은 좌표값을 주문 테이블(TBL_SO_M)에 업데이트
     * - XML ID: updateOrderCoord
     * - 용도: 구글 API나 캐시 테이블에서 찾은 좌표를 원본 주문에 반영
     * - 반환값: 업데이트된 행의 개수 (int)
     */
    int updateOrderCoord(Erp101001VO vo);
}
